...
class FileList(pygame.sprite.Group):
    ...
    def update_items(self):
        ...
        if(self.__item_constructor != None):
            ...
            self.items =  _all [(self.page - 1) * FileList.ITEMS_PER_PAGE: self.page *  FileList.ITEMS_PER_PAGE]
        else:
            self.items = []

        self.update_counter( math.ceil(len(_all) / FileList.ITEMS_PER_PAGE), self.page)
        
        for idx, itm in enumerate(self.items_pool):    
            txt = None
            if(idx < len(self.items)):
                txt = str(self.items[idx])[0:30]
            itm.set_text(txt)
    ...
    def selected(self, val):
        ...
        self.items_pool[self.__selected % FileList.ITEMS_PER_PAGE].set_color(FileList.ITEM_DEF_COLOR)
        ...
        self.items_pool[self.__selected % FileList.ITEMS_PER_PAGE].set_color(FileList.ITEM_SELECTED_COLOR)
